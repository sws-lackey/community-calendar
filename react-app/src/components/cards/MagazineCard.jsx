...
export default function MagazineCard({ event, filterTerm, onCategoryFilter }) {
  ...
  return (
    <>
      <div className="mb-3">
        <div className="rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-200 bg-white border border-gray-200">
          ...
          {event.image_url ? (
            <div className="relative h-[200px]">
              ...
              <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-lg px-2 py-1 text-center shadow-sm">
                <span className="block text-[10px] font-bold text-gray-600 leading-none">{dateParts.month}</span>
                <span className="block text-lg font-bold text-gray-900 leading-tight">{dateParts.day}</span>
              </div>
            ...
            <div className="relative px-4 pt-4 pb-3"
              ...
              <div className="flex items-start gap-3">
                <div className="bg-white/80 rounded-lg px-2 py-1 text-center shadow-sm flex-shrink-0">
                  <span className="block text-[10px] font-bold text-gray-600 leading-none">{dateParts.month}</span>
                  <span className="block text-lg font-bold text-gray-900 leading-tight">{dateParts.day}</span>
                </div>
                <div className="flex-1 min-w-0">
                  {event.url ? (
                    <a href={event.url} target="_blank" rel="noopener noreferrer"
                      className="text-base font-bold tracking-tight text-gray-900 hover:underline block leading-snug">
                      {event.title}
                    </a>
                  ) : (
                    <h5 className="text-base font-bold tracking-tight text-gray-900 leading-snug">{event.title}</h5>
                  )}
                  {timeStr && <p className="text-xs text-gray-600 mt-0.5">{timeStr}</p>}
                </div>
              </div>
            </div>
          )}
          ...
          <div className="px-4 py-3">
            ...
            {snippet && (
              <p className="text-sm text-gray-600 mt-1.5 cursor-pointer leading-snug" onClick={() => setShowDetail(true)}>{snippet}</p>
            )}
